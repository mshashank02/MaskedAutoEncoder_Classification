...
from transformers import ViTImageProcessor, ViTMAEForPreTraining, ViTModel, ViTMAEConfig
...
# Define and load a smaller MAE model from scratch
config = ViTMAEConfig(
    hidden_size=384,
    num_hidden_layers=6,
...
vit_encoder = ViTModel(config)  # same config as above
vit_encoder.load_state_dict(model.vit.state_dict(), strict=False)
...
results.to_csv("vitmae_probe_results_scratch.csv", index=False)
